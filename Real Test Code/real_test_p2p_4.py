...
right_location = LocationGlobalRelative(home_location.lat, home_location.lon + (right_distance / (111111 * math.cos(math.radians(home_location.lat)))), home_location.alt)
vehicle.simple_goto(right_location)

# Wait until reaching the right location
while True:
    distance = math.sqrt((vehicle.location.global_relative_frame.lat - right_location.lat)**2 +
                         (vehicle.location.global_relative_frame.lon - right_location.lon)**2)
    if distance < 1:
...
forward_location = LocationGlobalRelative(home_location.lat + (forward_distance / 111111), home_location.lon + (right_distance / (111111 * math.cos(math.radians(home_location.lat)))), home_location.alt)
vehicle.simple_goto(forward_location)

# Wait until reaching the forward location
while True:
    distance = math.sqrt((vehicle.location.global_relative_frame.lat - forward_location.lat)**2 +
                         (vehicle.location.global_relative_frame.lon - forward_location.lon)**2)
    if distance < 1:
...
while True:
    distance = math.sqrt((vehicle.location.global_relative_frame.lat - left_location.lat)**2 +
                         (vehicle.location.global_relative_frame.lon - left_location.lon)**2)
    if distance < 1:
...
while True:
    distance = math.sqrt((vehicle.location.global_relative_frame.lat - home_location.lat)**2 +
                         (vehicle.location.global_relative_frame.lon - home_location.lon)**2)
    if distance < 1:
